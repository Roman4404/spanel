...
class Interface(QMainWindow): #Интерфейс
    ...
    def closeEvent(self, event):
        ...
        a_vvv = self.valuts_volums_verticalSlider.value()
        b_mvfm = max_vol_for_micro
        with open('./mainWindows/date/settings_app.txt', 'w', newline='', encoding="utf8") as f:
            print(device, file=f)
            if self.valuts_volums_verticalSlider.value() == '':
                print('99', file=f)
            else:
                print(a_vvv, file=f)
            if max_vol_for_micro == '':
                print('100', file=f)
            else:
                print(b_mvfm, file=f)
        event.accept()
...
if __name__ == '__main__':
    ...
    app.setWindowIcon(QIcon('./mainWindows/Interface/image/icon_v2.png'))
    ex = Interface()
    ex.show()
    sys.exit(app.exec())
